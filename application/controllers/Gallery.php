...
class Gallery extends CI_Controller
{
    ...
    {       
        ...
        //get data
        $data['galleries'] = $this->gallery_model->fields('id, name, slug, description, thumbnail, status, created_at')->order_by('created_at', 'DESC')->paginate($this->_PER_PAGE, $total, $page);
        $data['pagination'] = $this->generate_pagination(site_url('gallery'), $total);

        $this->template->set_layout('layouts/main');
        $this->template->set_content('pages/gallery', $data);
        $this->template->render();
    }

    public function detail($slug)
    {       
        $gallery = $this->gallery_model->where('slug', $slug)->with_videos('fields:url','where:`url` != \'\'')->with_images('fields:filename','where:`filename` != \'\'')->get();
        ...
        if($slug && $gallery) {
            ...
            $this->template->set_layout('layouts/main');
            $this->template->set_content('pages/gallery-detail', $data);
            $this->template->render();
        } else {
            redirect('/','refresh');
        }
    ...
    protected function _loadcss() {
        ...
        $this->template->set_css('swipebox.css');        
    }
    ...
    protected function _loadjs() {      
        ...
        $this->template->set_js('jquery.swipebox.min.js','header');        
    }
}